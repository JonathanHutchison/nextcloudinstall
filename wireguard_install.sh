...
get_ips () {
  echo "Input VPS Public IP address on your Oracle Cloud Instance Page under \"Public IP Address\"".
  echo "Default IP addresses & ports are in[square brackets]. If you don't want to change them, press enter when prompted."
  echo ""
...
create_server_config () {
  ...
  echo "Please enter them like below (comma separated, no spaces):"
  ...
  echo ""
  echo -e "${LCYAN}./wireguard_install.sh Local $PK_FOR_CLIENT $PUBLIC_IP $WG_SERVER_IP $WG_CLIENT_IP $WGPORT $PORTLIST${NC}"
  echo ""
...
ask_firewall () {
  if [[ $1 == 1 ]]; then
    ...
    if [[ ! "$UFW_YN" =~ ^([yY][eE][sS]|[yY]|"")$ ]]; then
      echo -e "Limit access to your server."
      exit
    else
      clear_firewall
      setup_firewall
...
echo -e "***************************************************${NC}"
echo ""
echo "Install and configure wireguard on your machines."
if [[ $1 == "Local" ]]; then
  stop_wireguard
  update_system
  install_required
